...
void joystick::run()
{
    //emit this->connection("ok okokokok");
        short keeprunning = 1;
            ...
            while (keeprunning)
            {
                ...
                if (joyGetNumDevs()==0)
                {
                    ...
                    emit this->connection("Veuillez connecter le joystick ");
                    Sleep(5200);
                    break;
                }
                ...
                lastJoyState.dwFlags=JOY_RETURNALL | JOY_RETURNPOVCTS | JOY_RETURNCENTERED | JOY_USEDEADZONE;
                    ...
                    while(keeprunning)
                    {
                        emit this->ok("Joystick connecter ");
                        ...
                        if (cur_b==15)
                        {
                            emit this->ok("Fin du programme ");
                            fprintf(stdout, "End program button sequence initiated\n");
                            keeprunning=0;
                        }
                    }

                    Sleep(1000); // wait 10ms between checks for the next sample
                }
   }
